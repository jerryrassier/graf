...
/*
  grafmodule.c Copyright (C) 2016 Jerry Rassier - adapted from zran.c,
  for which the copyright/license notice appears below.
...
static int extract(FILE *in, struct access *index, off_t offset, unsigned char *buf, int len)
{
    ...
    if (len <= 0)
    {
        fprintf(stderr, "graf: nonsenical number of bytes (%d) requested. Don't ask for more than %d (max int size on this system) or fewer than 1 (that's just silly).\n", len, INT_MAX);
        return 0;
	}

	if((offset + len) > index->usize)
	{
		// Attempt to read beyond the end of the uncompressed data
...
static PyObject* graf_build_index(PyObject *self, PyObject *args)
{
	char *gzfile;
	char *idxfile;
	size_t spanlength;
	
	if(!PyArg_ParseTuple(args, "ssn", &gzfile, &idxfile, &spanlength))
	{
		return NULL;
	}
...
static PyObject* graf_extract(PyObject *self, PyObject *args)
{
	char *gzfile;
	char *idxfile;
	off_t readoffset;
	...
	if(!PyArg_ParseTuple(args, "ssnn", &gzfile, &idxfile, &readoffset, &readlen))
	{
	...
	int idxloadresult = load_index(idxfile, &index);
	
	if(idxloadresult != 0)
	{
		fprintf(stderr, "graf: unable to load index file %s", idxfile);
		free(index);
		return Py_BuildValue("");
	}
...
static PyObject* graf_read_index_offsets(PyObject *self, PyObject *args)
{
	char *idxfile;
	struct access* idx = malloc(sizeof(*index));
	PyObject *ret, *item;
	
	if(!PyArg_ParseTuple(args, "s", &idxfile))
	{
		free(idx);
		return PyList_New(0);
	}
	
	int idxloadresult = load_index(idxfile, &idx);
	
	if(idxloadresult != 0)
	{
		fprintf(stderr, "Unable to load index file %s\n", idxfile);
		free(idx);
		return PyList_New(0);
	}

	ret = PyList_New(idx->have);
	...
	while(i < idx->have)
	{
		struct point* pt;
		pt = &idx->list[i];
		item = Py_BuildValue("nni", pt->out, pt->in, pt->bits);
...
static PyObject* graf_get_usize(PyObject *self, PyObject *args)
{
	char *idxfile;
	struct access* idx = malloc(sizeof(*idx));
	...
	free_index(idx);
	return Py_BuildValue("n",idx->usize);
}

// Python method table
static PyMethodDef GrafMethods[] = {
	{"build_index", graf_build_index, METH_VARARGS, "Build an index file"},
	{"extract", graf_extract, METH_VARARGS, "Extract uncompressed data from a file"},
	{"read_index_offsets", graf_read_index_offsets, METH_VARARGS, "Read tuples from an index of format (uoffset, coffset, bits)"},
	{"get_usize", graf_get_usize, METH_VARARGS, "Get uncompressed size of the gzipped file to which the provided index file corresponds"},
	{NULL, NULL, 0, NULL}
};

PyMODINIT_FUNC initgraf(void)
{
	(void) Py_InitModule("graf", GrafMethods);
}
